...
백엔드를 Flask·FastAPI 같은 웹 프레임워크 대신 **Python 내장 `http.server`로 직접** 구현했습니다. 프레임워크가 나빠서가 아니라, 이 앱의 요구사항에 내장 모듈이 더 맞았기 때문입니다.

- **무설치 배포가 핵심 요구사항** — 개발을 모르는 팀원도 쓸 앱이라서 `pip install`로 별도 패키지를 받는 과정 자체가 장벽입니다(회사 PC의 프록시·방화벽, 권한, 버전 충돌, 외부망 연결 한계등 실패 지점). `http.server`·`sqlite3`는 Python에 기본 내장되어 있어 **"Python 설치 → 실행.bat 더블클릭"** 으로 끝납니다.
- **규모에 맞는 선택** — 사용자 1명, API 10여 개, 단일 SQLite 파일 규모입니다. 프레임워크의 강점(대규모 라우팅, 다중 사용자, 자동 문서화, 비동기)이 필요 없어, 이 규모에선 얻는 이득보다 설치·관리 부담이 큽니다.
- **관리 포인트 최소화** — 외부 의존성이 없으면 패키지 취약점 추적·버전 관리 부담이 사라지고, Python 자체만 관리하면 됩니다.

**트레이드오프** — 대신 라우팅을 `if/elif`로 직접 분기해야 해 코드가 길고, 입력값 검증·API 문서 자동화 같은 편의 기능은 없습니다. 다중 사용자가 동시에 쓰는 실서버로 확장한다면 그때는 FastAPI 등으로 이전하는 것이 적절합니다.
