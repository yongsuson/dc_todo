...
- **무설치 배포가 핵심 요구사항** — 개발을 모르는 팀원에게 배포하는 앱이라, `pip install`로 별도 패키지를 받는 과정 자체가 장벽입니다(회사 PC의 프록시·방화벽, 권한, 버전 충돌 등 실패 지점). `http.server`·`sqlite3`는 Python에 기본 내장되어 있어 **"Python 설치 → 실행.bat 더블클릭"** 으로 끝납니다.
